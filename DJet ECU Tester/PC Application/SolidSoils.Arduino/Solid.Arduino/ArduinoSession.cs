...
namespace Solid.Arduino
{
    ...
    public class ArduinoSession : IFirmataProtocol, IServoProtocol, II2CProtocol, IStringProtocol, IDisposable
    {
        ...
        private void ProcessSysExMessage(byte messageByte)
        {
            ...
            switch ((SysExCommand)_messageBuffer[1])
            {
                ...
                default: // Unknown message
                    DeliverMessage(CreateSysExMessage());
                    return;
            }
        }
        ...
        private FirmataMessage CreateSysExMessage()
        {
            byte[] Buffer = new byte[((_messageBufferIndex - 2) / 2) + 1];
            int Offset = 0;
            Buffer[Offset++] = _messageBuffer[1];
            for (int x = 2; x < _messageBufferIndex; x += 2)
            {
                Buffer[Offset++] = (byte)(_messageBuffer[x] | (_messageBuffer[x + 1] << 7));
            }

            return new FirmataMessage(Buffer, MessageType.SysEx);
        }
